...
export const lanczosResize = (srcData, srcW, srcH, dstW, dstH, { addLog = () => { } } = {}) => {
  addLog(`\n🔷 LANCZOS3 ALGORITHM`);
  addLog(`Target size: ${dstW}×${dstH}`);
  addLog(`Using 6×6 pixel kernel (36 samples per pixel)`);
  addLog(`Applying windowed sinc function`);
  ...
  for (let y = 0; y < dstH; y++) {
    for (let x = 0; x < dstW; x++) {
      ...
      for (let c = 0; c < 4; c++) {
        ...
        for (let ky = 0; ky < filterSize * 2; ky++) {
          for (let kx = 0; kx < filterSize * 2; kx++) {
            ...
            const weight =
              lanczosKernel(srcX - srcXi) * lanczosKernel(srcY - srcYi);
      ...
      if (samplesShown < 1) {
        addLog(`  Dst(${x},${y}) samples 6×6 grid via sinc function`);
        samplesShown++;
      }
    }
  }

  addLog(`✓ Completed high-quality resize of ${dstW * dstH} pixels`);
  return dstData;
};