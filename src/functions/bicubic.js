...
const bicubicWeight = (x) => {
  ...
  if (absX <= 1) {
    ...
    return (
      a * absX * absX * absX - 5 * a * absX * absX + 8 * a * absX - 4 * a
    );
  }
  return 0;
};

export const bicubicInterpolation = (srcData, srcW, srcH, dstW, dstH, { addLog = () => { } } = {}) => {
  addLog(`\n🔷 BICUBIC INTERPOLATION`);
  addLog(`Target size: ${dstW}×${dstH}`);
  addLog(`Using 4×4 pixel kernel (16 samples per pixel)`);
  ...
  for (let y = 0; y < dstH; y++) {
    for (let x = 0; x < dstW; x++) {
      ...
      for (let c = 0; c < 4; c++) {
        ...
        for (let dy = -1; dy <= 2; dy++) {
          for (let dx = -1; dx <= 2; dx++) {
            ...
            const weight =
              bicubicWeight(srcX - srcXi) * bicubicWeight(srcY - srcYi);
      ...
      if (samplesShown < 1) {
        addLog(
          `  Dst(${x},${y}) uses 4×4 grid around (${srcX.toFixed(
            1
          )},${srcY.toFixed(1)})`
        );
        samplesShown++;
  ...
  addLog(`✓ Applied cubic interpolation to ${dstW * dstH} pixels`);
  return dstData;
};